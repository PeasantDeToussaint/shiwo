 ...
 * Checks label uniqueness across all results so the content phase
...
function validateResultsPlan(plan, results) {
  ...
  for (const p of plan) {
    ...
    if (!Array.isArray(p.strengthLabels) || p.strengthLabels.length < 6)
      errors.push(`${p.id}: strengthLabels must have 6 items (got ${p.strengthLabels?.length ?? 0})`);
